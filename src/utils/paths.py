...
CHARACTERS = data("characters.json")  # 👥 registr postav (multi-character) + aktivní slot
...
MINIGAME_CURRENCY = data("minigame_currency.json")  # přepínač měny miniher (silver/gold)
...
CRATES_DIR  = os.path.join(ROOT, "src", "assets", "crates")
CARDS_DATA  = data("cards_data.json")
CARDS_INVENTORY = data("cards_inventory.json")
CARDS_CRATES = data("cards_crates.json")
