...
Překrývá PNG rámečka na kartu přesným alpha compositingem.
"""
...
def apply_frame_to_card(card_image_path: str, frame_id: str = None):
    """
    Aplikuje PNG rámeček NA VRCHU karty s alpha compositingem.
    
    Oběma obrázky jsou STEJNÉ rozměry (1429×2000).
    Frame je transparentní PNG, který se pozicuje na (0,0).
    ...
    try:
        # Otevři kartu a frame jako RGBA
        card_img = Image.open(card_image_path).convert("RGBA")
        frame_img = Image.open(frame_image_path).convert("RGBA")
        
        # DŮLEŽITÉ: Oba obrázky mají STEJNÉ rozměry!
        # Pokud ne, resize frame na velikost karty
        if frame_img.size != card_img.size:
            frame_img = frame_img.resize(card_img.size, Image.Resampling.LANCZOS)
        
        # Vytvořit obrázek se base layer (karta)
        result = Image.new("RGBA", card_img.size)
        result.paste(card_img, (0, 0))
        
        # Overlay frame NA VRCHU - přesný alpha compositing
        # Frame zůstane transparentní a bude vidět pod ním karta
        result.paste(frame_img, (0, 0), frame_img)
        
        # Převeď NA RGB pro Discord (bílé pozadí)
